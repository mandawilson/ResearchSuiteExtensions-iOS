...
open class RSNewSliderView: UIStackView {
    
    open class func newView(minimumValue: Int, maximumValue: Int, stepSize: Int = 1) -> RSNewSliderView? {
        let bundle = Bundle(for: RSNewSliderView.self)
        guard let views = bundle.loadNibNamed("RSNewSliderView", owner: nil, options: nil),
            let view = views.first as? RSNewSliderView else {
                return nil
        }
    ...
    open class func configureView(view: RSNewSliderView, minimumValue: Int, maximumValue: Int, stepSize: Int = 1) {
